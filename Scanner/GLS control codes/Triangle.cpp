...
static rlResult markSquare(rlHandle handle)
{
    ...
    if ((result = rlListAppendJumpAbs2D(list, 0, triangleSize / 2)) != rlSUCCESS) return result;
    if ((result = rlListAppendJumpAbs2D(list, -triangleSize / 2, -triangleSize / 2)) != rlSUCCESS) return result;
    if ((result = rlListAppendJumpAbs2D(list, triangleSize / 2, -triangleSize / 2)) != rlSUCCESS) return result;
    if ((result = rlListAppendJumpAbs2D(list, 0, triangleSize / 2)) != rlSUCCESS) return result;
